# CampDash — deploy to campdash.fun (Oracle Always-Free VM, single deploy)

One container stack serves the mobile UI (`/`), the API, photos (`/media`), and `/admin` from one
origin — a QR code → `https://campdash.fun` → menu → order → photo in your inbox. Verified to build
and run via `docker compose`.

```
Phone scans QR → https://campdash.fun ──(Cloudflare DNS+HTTPS)──► Oracle VM (docker compose)
...
1. **Domain:** register **campdash.fun** at Porkbun or Cloudflare Registrar (~$1–3). Add it to a
...
# CD_SMTP_* (your Gmail + App Password), CD_MAIL_FROM, CD_CORS_ORIGINS=["https://campdash.fun"]
...
## E. Put HTTPS in front (campdash.fun → :8080)
...
```
Point the Cloudflare DNS A record for `campdash.fun` at the VM's public IP. Done — HTTPS is automatic.

## F. QR codes
Generate a QR pointing at **https://campdash.fun/** and print it on table tents / signage. Scanning
goes straight to the mobile menu.

## Operate
- **Admin:** `https://campdash.fun/admin` → enter `CD_ADMIN_KEY` → see every order (item, email,
